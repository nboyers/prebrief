...
1. **OAuth client**: do you want to register your own Google Cloud OAuth
   Desktop client (recommended for privacy — your data never flows
   through a client ID I control), or should I ship one with the app
   for convenience? I default to "you create your own" with a 5-step
   README.
2. **LLM default**: Anthropic + `claude-sonnet-4-5` OK as default? Switchable
   in settings.
3. **Granola Personal API access**: which plan are you on? The Personal
   API needs a paid tier (Business or above for MCP; Personal API has
   similar gating). If you can:
   a. Log in to https://app.granola.ai → Settings → look for
   "Personal API" or "Developer" → generate a key starting with `grn_`,
   and confirm it works by running:
   `curl -H "Authorization: Bearer grn_…" https://api.granola.ai/v1/notes?limit=1`
   b. If your plan doesn't expose it, tell me and I'll spec out a
   degraded "calendar-only" mode and we'll wait until you have access
   to ship the briefing feature.
4. **Final build host**: I'll develop in this Linux workspace, but the
   final `.dmg` must be produced on a Mac (cross-building Mac apps from
   Linux is unreliable). You'll run `pnpm dist` on your Mac to produce
   the installer. OK?
5. **Auto-start at login**: want the app to launch on login by default?

---

## 8. What I'll do once you approve

1. Scaffold M1 in `/home/coder/meeting-briefer` and push to a GitHub repo
   under your account (you tell me the name).
2. Stop and wait at each milestone boundary for a quick check-in.
